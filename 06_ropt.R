...
## Title: Rock Ptarmigan Time Removal Analysis

## Author: Michaela L. Gustafson

## Description: Analyzing only ROPT observations in 2019&2021
...
head(ropt)
# now 'ropt' df represents counts for ROPT
...
ropt.wide$id <- as.factor(ropt.wide$id)
colnames(ropt.wide)


# ?unmarked

timeints <- ropt.wide[,c(2:6)]
...
fm0.two <- multinomPois(~ 1 ~ 1, data = roptFrame2) #null model
fm.two.full <- multinomPois( ~ julian + min_after_sun + observer + tempf + sky + hear ~ 1, data = roptFrame2)
fm.two.jul <- multinomPois( ~ julian + min_after_sun + observer + sky + hear ~ 1, data = roptFrame2)
fm.two.temp <- multinomPois( ~ min_after_sun + observer + tempf + sky + hear ~ 1, data = roptFrame2)
...
write.csv(output.twomin, here("output/ropt_2min_output.csv"))
