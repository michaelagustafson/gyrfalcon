...
```{r dem}
ak_dem <- raster(here("data/DEM_Michaela/DEM_Michaela/dsm_mosaic_AK.tif"))
plot(ak_dem)
...
1. Reproject land cover raster to be same CRS, origin, extent as elevation 

```{r reproject}
# reproject land cover crs to elevation crs
crs(lc.stack4[[2]])
lc.stack5 <- projectRaster(e.focal, crs = crs(lc.stack))

lc.stackcrop <- raster::crop(lc.stack)
# crop so extents really are the same (ran into some issues later on when trying to stack)
r.reproj <- raster::crop(e.focal, r.reproj)
# check to make sure stats are the same
extent(ak_dem) == extent(r.reproj)
crs(r.reproj)
crs(ak_dem)
origin(ak_dem) == origin(r.reproj)
```
...
# 2019 points
points19.proj <- st_transform(points19.sf, crs(lc.stack3))
...
points21.proj <- st_transform(points.21.sf, crs(lc.stack3))
